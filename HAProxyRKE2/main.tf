...
data "template_file" "rke2_token" {
  ...
  vars = {
    ...
    san-3            = "${var.vm_haproxy["ip"]}"
    san-4            = "${var.vm_haproxy["name"]}"
    san-5            = "${var.vm_haproxy["fqdn"]}"
  }
}
...
resource "vsphere_virtual_machine" "haproxy" {
  name             = var.vm_haproxy["name"]
  resource_pool_id = data.vsphere_resource_pool.pool.id
  datastore_id     = data.vsphere_datastore.datastore.id
  folder           = var.vsphere_folder

  num_cpus = var.haproxycpucount
  memory   = var.haproxymemory
  ...
  disk {
    label            = "${var.vm_haproxy["name"]}.vmdk"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = var.vm_haproxy["name"]
        domain    = var.vsphere_network_ips["domain"]
      }

      network_interface {
        ipv4_address = var.vm_haproxy["ip"]
  ...
  provisioner "file" {
    ...
    source      = "haproxy/certificate.pem.key"
    destination = "/tmp/certificate.pem.key"
  }
